...
// where 1,1 is the bottom-left of the chessboard and 8,8 is the top-right). 
...
// return the first queen in the list that is attacking another piece in the same format it was provided.

// For example: if strArr is ["(2,1)", "(4,2)", "(6,3)", "(8,4)", "(3,5)", "(1,6)", "(7,7)", "(5,8)"] 
// then your program should return the string true. The corresponding chessboard of queens for this 
// input is below (taken from Wikipedia).

function EightQueens(strArr) { 
  for(let i=0; i<strArr.length-1; i++){
    var x = parseInt(strArr[i].substring(1,2));
    var y = parseInt(strArr[i].substring(3,4));
    for(let j=i+1;j<strArr.length;j++){
      var xj = parseInt(strArr[j].substring(1,2));
      var yj = parseInt(strArr[j].substring(3,4));
      if(x==xj || y == yj){
        return `(${x},${y})`;
      } else if(x == y && xj == yj){
        return `(${x},${y})`;
        ...
        return `(${x},${y})`;
      } else if(Math.abs(xj-x) == Math.abs(yj-y)) {
        return `(${x},${y})`;
      }
    }
  }
